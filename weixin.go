...
func PostTextmy(touser string, text string, token string) error {
	var msg struct {
		ToUser  string `json:"touser"`
		MsgType string `json:"msgtype"`
		Text    struct {
	...
	msg.MsgType = "text"
	msg.Text.Content = text
	return postMessagemy(token, &msg)
}
...
func (wx *Weixin) AddTemplate(shortid string) (string, error) {
	...
	reply, err := postRequest(weixinTemplate+"/api_add_template?access_token=", wx.tokenChan, data)
	if err != nil {
		return "", err
	}
...
func PostTemplateMessagemy(touser string, templateid string, url string, data TmplData, token string, mini map[string]string) (int32, error) {
	var msg struct {
		ToUser     string            `json:"touser"`
		TemplateId string            `json:"template_id"`
		Url        string            `json:"url,omitempty"`
		Data       TmplData          `json:"data,omitempty"`
		Mini       map[string]string `json:"miniprogram,omitempty"`
	}
	msg.ToUser = touser
	msg.TemplateId = templateid
	msg.Url = url
	...
	//fmt.Println(string(msgStr), "dddddddddddddddddddddddddddddddddd")
	reply, err := postRequestmy(weixinHost+"/message/template/send?access_token=", token, msgStr)
	if err != nil {
		return 0, err
	}
	var resp struct {
...
func (wx *Weixin) createAccessToken(c chan accessToken, appid string, secret string) {
	...
	for {
		...
		fmt.Println(token, "dddddddddddddddddddddddddddddddddddddddddddddddddddddddddd")
		c <- token
	}
}
...
func postRequestmy(reqURL string, token string, data []byte) ([]byte, error) {
	for i := 0; i < 3; i++ {
...
func postMessagemy(token string, msg interface{}) error {
	data, err := marshal(msg)
	if err != nil {
		return err
	}
	_, err = postRequestmy(weixinHost+"/message/custom/send?access_token=", token, data)
	return err
}
